...
void WriterSQL::updateDB(int current_version)
{
    ...
    case 0 :
        ...
        l_query.exec("CREATE TABLE IF NOT EXISTS ic_events(event_time DATETIME DEFAULT CURRENT_TIMESTAMP,hwip INTEGER NOT NULL,room_name TEXT,char_name TEXT,ic_name TEXT,message TEXT NOT NULL,FOREIGN KEY (hwip) REFERENCES hwips(hwip)ON DELETE CASCADE)");
        l_query.exec("CREATE TABLE IF NOT EXISTS room_event_types(type_id INTEGER PRIMARY KEY,type_name TEXT NOT NULL UNIQUE)");
        l_query.exec("CREATE TABLE IF NOT EXISTS room_events(event_id PRIMARY KEY,event_time DATETIME DEFAULT CURRENT_TIMESTAMP,hwip INTEGER NOT NULL,target_hwip INTEGER,room_name TEXT,char_name TEXT,ooc_name TEXT,event_subtype INTEGER NOT NULL,message TEXT,FOREIGN KEY (hwip) REFERENCES hwips(hwip)ON DELETE CASCADE,FOREIGN KEY (target_hwip) REFERENCES hwips(hwip)ON DELETE CASCADE,FOREIGN KEY (event_subtype) REFERENCES room_event_types(type_id))");
        l_query.exec("CREATE TABLE IF NOT EXISTS login_events(event_time DATETIME DEFAULT CURRENT_TIMESTAMP,hwip INTEGER NOT NULL,profile_name TEXT,FOREIGN KEY (hwip) REFERENCES hwips(hwip)ON DELETE CASCADE)");
        l_query.exec("CREATE TABLE IF NOT EXISTS connect_events(event_time DATETIME DEFAULT CURRENT_TIMESTAMP,hwip INTEGER NOT NULL,hdid TEXT NOT NULL,failed INTEGER DEFAULT 0,FOREIGN KEY (hwip) REFERENCES hwips(hwip)ON DELETE CASCADE)");
